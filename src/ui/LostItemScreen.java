...
public class LostItemScreen extends JPanel {
    ...
    private List<LostItem> currentItems;
    private List<LostItem> allItems;

    private boolean updatingFilters = false;


    public LostItemScreen(MainFrame parent) {
        this.parent = parent;
        ...
        reportButton = UIUtils.createStyledButton("Report New Lost Item", new Color(60, 179, 113));
    ...
    private void createItemTable() {
        ...
        String[] columnNames = {"ID", "Item Name", "Location", "Date Lost", "Description"};
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row >= 0 && row < currentItems.size()) {
                        LostItem selectedItem = currentItems.get(row);
    ...
    private void searchItems(String keyword) {
        ...
        currentItems = dao.searchLostItems(keyword);
        updateTableData();
    }
    ...
    private void showItemDetails(LostItem item) {
        ...
        details.append("Date Lost: ").append(DateUtils.formatDate(item.getDateLost())).append("\n\n");
    ...
    private void updateTableData() {
        ...
        for (LostItem item : currentItems) {
            Object[] rowData = {
                item.getLostItemID(),
                item.getTitle(),
                item.getLocation(),
                DateUtils.formatDate(item.getDateLost()),
                item.getDescription()
            };
        ...
        try {
            ...
            for (LostItem item : allItems) {
                ...
                if (item.getDateLost() != null) {
                    dates.add(DateUtils.formatDate(item.getDateLost()));
                }
            }
    ...
    private void applyFilters() {
        ...
        for (LostItem item : allItems) {
            ...
            if (!dateValue.equals("All Dates")) {
                String itemDate = DateUtils.formatDate(item.getDateLost());
    ...
    private void deleteSelectedItem() {
        ...
        if (selectedRow >= 0 && selectedRow < currentItems.size()) {
            LostItem selectedItem = currentItems.get(selectedRow);
    ...
    private void deleteItem(LostItem item) {
        int confirm = JOptionPane.showConfirmDialog(
            this,
            "Are you sure you want to delete this lost item?\n" +
            "Item: " + item.getTitle() + "\n" +
            "Date: " + DateUtils.formatDate(item.getDateLost()),
            "Confirm Deletion",
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            LostItemDAO dao = new LostItemDAO();
            boolean success = dao.deleteLostItem(item.getLostItemID());
    ...
    private void editItem(LostItem item) {
        ...
        editDialog.setTitle("Edit Lost Item");
        ...
        JLabel dateLabel = new JLabel("Date Lost (dd/mm/yyyy):");
        JTextField dateField = new JTextField(DateUtils.formatDate(item.getDateLost()), 10);
        ...
        saveButton.addActionListener(e -> {
            ...
            try {
                ...
                item.setDateLost(DateUtils.parseDate(dateStr));
                item.setDescription(description);


                LostItemDAO dao = new LostItemDAO();
                boolean success = dao.updateLostItem(item);
